...
class GaussianLaplace:
    def __call__(self, image):
        image_np = image.squeeze(0).cpu().numpy().astype(np.uint8)
        laplace = cv2.Laplacian(cv2.GaussianBlur(image_np, (0, 0), 1), cv2.CV_64F)
        return torch.from_numpy(laplace).float()

class GaussianGradientMagnitude:
    def __call__(self, image):
        image_np = image.squeeze(0).cpu().numpy().astype(np.uint8)
        gradient_x = cv2.Sobel(cv2.GaussianBlur(image_np, (0, 0), 2), cv2.CV_64F, 1, 0)
        gradient_y = cv2.Sobel(cv2.GaussianBlur(image_np, (0, 0), 2), cv2.CV_64F, 0, 1)
...
class Sobel:
    def __call__(self, image):
        ...
        if max_sobel == 0:
            max_sobel = 1
        sobel = (sobel / max_sobel) * 255.0
        return torch.from_numpy(sobel).float()

class Canny:
    def __call__(self, image):
        image_np = image.squeeze(0).cpu().numpy().astype(np.uint8)
        canny_edges = cv2.Canny(image_np.astype(np.uint8), 100, 200)
        return torch.from_numpy(canny_edges).float()
...
transform_laplace = transforms.Compose([
        transforms.Grayscale(num_output_channels=3),
        SquarePad(),
        transforms.Resize(256),
        transforms.RandomHorizontalFlip(),
...
transform_sobel = transforms.Compose([
        transforms.Grayscale(num_output_channels=3),
        SquarePad(),
        transforms.Resize(256),
        transforms.RandomHorizontalFlip(),
...
def get_vetdata(root, l_samples, u_samples, name,
                transform_train=transform_train,
                transform_laplace=transform_laplace,
                transform_sobel=transform_sobel,
    ...
    if 'remix' in name:
        train_labeled_dataset = Bone_labeled(root_dir=os.path.join(root, 'Training'),
                                         indexs=train_labeled_idxs, transform=transform_laplace)
    else:
        train_labeled_dataset = Bone_labeled(root_dir=os.path.join(root, 'Training'),
    ...
    if 'remix' in name or 'fix' in name:
        train_unlabeled_dataset = Bone_unlabeled(root_dir=os.path.join(root, 'Training'),
                                                indexs=train_unlabeled_idxs, transform=TransformTwice(transform_train, transform_laplace, transform_sobel))
    else:
        train_unlabeled_dataset = Bone_unlabeled(root_dir=os.path.join(root, 'Training'),
