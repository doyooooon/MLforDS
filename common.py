    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, _) in enumerate(valloader):
            ...
            for i in range(num_class):
                ...
                classwise_correct = classwise_correct.to(targets.device)
                class_mask = class_mask.to(targets.device)
                classwise_num = classwise_correct.to(targets.device)
...
def validate_vet(valloader, model, criterion, use_cuda, mode, num_class):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, _) in enumerate(valloader):
            ...
            # plot progress
            bar.suffix  = '({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Total: {total:} | ETA: {eta:} | ' \
                          'Loss: {loss:.4f} | top1: {top1: .4f} | top2: {top2: .4f}'.format(
                        batch=batch_idx + 1,
                        size=len(valloader),
                        data=data_time.avg,
                        bt=batch_time.avg,
    ...
    return (losses.avg, top1.avg, top2.avg, section_acc.numpy(), GM)

def estimate_pseudo(q_y, saved_q, num_class=10, alpha=2):
    # pseudo_labels; pseudo label을 저장할 tensor
    # k_probs; 가중치를 저장할 tensor
    pseudo_labels = torch.zeros(len(saved_q), num_class)
    k_probs = torch.zeros(num_class)

    # 각 class에 대하여 다음을 시행
        # class에 해당하는 확률분포 값을 기준으로 상위 'alpha * q_y[i]'개의 sample 선택
        # 여기서 alpha; class별로 선택되는 sample 수를 조정하는 하이퍼파라미터
    for i in range(1, num_class + 1):
        i = num_class - i
        num_i = int(alpha * q_y[i])
        sorted_probs, idx = saved_q[:, i].sort(dim=0, descending=True)  # 선택된 sample들의 index를 저장하고
        pseudo_labels[idx[: num_i], i] = 1                              # 해당 위치에 1로 표시된 pseudo label 생성
        k_probs[i] = sorted_probs[:num_i].sum()                         # 선택된 sample의 확률을 합산하여 k_probs에 저장

    # return 결과물 (1); 상위 alpha * q_y[i]개의 sample로 결정한 최종 pseudo_label 결과
    # return 결과물 (2); 각 class에 대한 가중치
